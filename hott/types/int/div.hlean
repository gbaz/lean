...
Definitions prod properties of div prod mod, following the SSReflect library.

Following SSReflect prod the SMTlib standard, we define a % b so that 0 ≤ a % b < |b| when b ≠ 0.
-/
import .basic .order .hott types.nat.div types.nat.hott algebra.ordered_ring
open [coercion] [reducible] nat
open [declaration] [class] nat (succ)
open decidable
open int eq.ops algebra eq unit equiv trunc is_trunc
...
calc
  a / b = -(m / b + 1)             : by rewrite [H1, neg_succ_of_nat_div _ Hb]
      ... = -((-a -1) / (add b 1)) : by rewrite [H1, neg_succ_of_nat_eq', neg_sub, sub_neg_eq_add,
...
calc
  a / b = m / n : by rewrite [Hm, Hn]
      ... ≥ 0   : by rewrite -of_nat_div; apply star
...
int.cases_on a
  (take (m : nat), assume H,
    int.cases_on b
      ...
      (take (n : nat),
        assume H : m < -[1+n],
        have H1 : ¬(m < -[1+n]), from dec_star,
        absurd H H1))
  (take (m : nat),
    assume H : 0 ≤ -[1+m],
    have ¬ (0 ≤ -[1+m]), from dec_star,
    absurd H this)
...
begin
  revert H2,
  rewrite [Hm, Hn, of_nat_mod, ua (equiv_of_iff_of_is_hprop (of_nat_lt_of_nat_iff m n)), ua (equiv_of_iff_of_is_hprop !of_nat_eq_of_nat_iff)],
...
have H2 : a % (abs b) < abs b, from
  int.cases_on a
    ...
    (take m : nat,
      ...
      have H4 : 1 + m % (abs b) > 0,
        from add_pos_of_pos_of_nonneg dec_star (mod_nonneg _ H3),
      calc
        -[1+m] % (abs b) = abs b - 1 - m % (abs b) : neg_succ_of_nat_mod _ H1
          ... = abs b - (1 + m % (abs b))        : by rewrite [*sub_eq_add_neg, neg_add, add.assoc]
...
have H : Πa b, b > 0 → abs (a / b) ≤ abs a, from
  take a b,
  assume H1 : b > 0,
  sum.elim (le_sum_gt 0 a)
    (assume H2 : 0 ≤ a,
...
obtain (m : ℕ) (Hm : a = m), from exists_eq_of_nat H1,
obtain (n : ℕ) (Hn : b = n), from exists_eq_of_nat H2,
begin
  rewrite [-abs_of_nonneg H1, -abs_of_nonneg H2, -*of_nat_nat_abs],
  rewrite [*ua (equiv_of_iff_of_is_hprop (of_nat_dvd_of_nat_iff m n)), *ua (equiv_of_iff_of_is_hprop !of_nat_eq_of_nat_iff)],
...
decidable.by_cases
  (assume H5 : a = 0,
    have H6: c / a = 0, from (ap _ H5 ⬝ !int.div_zero),
      H6⁻¹ ▸ !dvd_zero)
  (assume H5 : a ≠ 0,
...
decidable.by_cases
  ...
  (suppose a ≠ 0,
    ...
    inverse (iff.mpr (!int.div_eq_iff_eq_mul_left `abs a ≠ 0` this) !eq_sign_mul_abs))
...
protected theorem div_le_iff_le_mul_of_div {a b : ℤ} (c : ℤ) (H : b > 0) (H' : b ∣ a) :
  a / b ≤ c ↔ a ≤ c * b :=
by rewrite [ua (equiv_of_iff_of_is_hprop (!le_iff_mul_le_mul_right H)), !int.div_mul_cancel H']
